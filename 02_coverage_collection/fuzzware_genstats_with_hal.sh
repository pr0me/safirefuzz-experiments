...
TARGET_DIR=$DIR/fuzzware_hal_data
...
for t in $TARGETS; do
    for j in {0..5} ; do
        echo fuzzware genstats -p $DIR/../01_fuzzing/fuzzware/firmware/$t/fuzzware-project-run-0$j --valid-bb-file $DIR/valid_bb_files/$t.elf_bbs.txt -f
    done
...
for csv in `find . | grep covered_bbs_by_second_into_experiment.csv`; do
    data_name=$(echo $csv | sed 's/\/fuzzware-project-run-/_/g' | sed 's/\/stats.*/.data/g' | sed 's/\.\//fuzzware_hal_/g')
